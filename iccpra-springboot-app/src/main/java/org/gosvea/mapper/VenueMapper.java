...
//    void add(Venue venue);
    //修改后的
    @Insert("insert into venue(id,state, city, address, time_zone, cancellation_policy, payment_mode, nonrefundable_fee, fob_key, deposit, membership_fee, usage_fee, refundable_status, book_method, registration_link,venue_status,icpis_manager,cpr_price,bls_price,cpradult_price,cprinstructor_price) " +
            "values(#{id},#{state}, #{city}, #{address}, #{timeZone}, #{cancellationPolicy}, #{paymentMode}, #{nonrefundableFee}, #{fobKey}, #{deposit}, #{membershipFee}, #{usageFee}, #{refundableStatus}, #{bookMethod}, #{registrationLink},#{venueStatus},#{icpisManager},#{cprPrice},#{blsPrice},#{cpradultPrice},#{cprinstructorPrice})")
    void add(Venue venue);
